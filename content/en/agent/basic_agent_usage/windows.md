...
The core and APM/trace components of the Windows Agent run under the `agent user` account. The Live Processes component, if enabled, runs under the `LOCAL_SYSTEM` account. You can learn more about the [Datadog Windows Agent User][3].
...
- Automated password management: The passwords for gMSAs are handled at the operating system level, and are rotated regularly without requiring manual intervention.
...
| `DDAGENTUSER_NAME`                          | String  | Override the default `agentuser` username used during Agent installation _(v6.11.0+)_. [Learn more about the Datadog Windows Agent User][3].                                                                                      |
| `DDAGENTUSER_PASSWORD`                      | String  | Override the cryptographically secure password generated for the `agentuser` user during Agent installation _(v6.11.0+)_. Must be provided for installs on domain servers. [Learn more about the Datadog Windows Agent User][3].  |
